...
A second skill (or a `--verify` flag) for checking whether `.yunxing/agents/*.md` artifacts still match the seed-template schema would duplicate work the existing setup skill already handles in conversation.
