...
- **[new-raw](./new-raw/SKILL.md)**: Capture a raw requirement exactly as given (no clarification, no alignment, no dialogue). Creates a GitHub/GitLab issue (so you can paste images) or a local markdown file as fallback.
- **[sync-ups](./sync-ups/SKILL.md)**: Sync this fork forward from upstream, pulling in new and changed skills incrementally.
- **[vision](./vision/SKILL.md)**: Send an image to a configured external vision provider when native vision isn't available or sufficient.
...
- **[report-from-commits](./report-from-commits/SKILL.md)**: Turn git history since an exact date into a copy-pastable, non-technical report grouped by feature. Use for client updates, stakeholder recaps, or progress reports. Refuses to guess dates.
