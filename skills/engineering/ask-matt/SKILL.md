...
- **`/report-from-commits`** — turn git history since a date into a copy-pastable, non-technical report grouped by feature, with the total commit count and each involved author's commit count and percentage in its opening line. Reach for it when you need a client update, stakeholder recap, or progress report from commits. Accepts relative dates ("最近一个礼拜", "last week") and resolves them automatically; only asks for clarification on anchorless phrases like "recently".
...
- **`/new-raw`** — capture a requirement exactly as it arrived, without clarification or dialogue, into the configured issue tracker or a local Markdown fallback. Use it when preservation comes before refinement; `/triage` can shape the captured issue later.
...
- **`/sync-ups`** — periodic fork maintenance that absorbs incremental changes from `mattpocock/skills`, preserving local branding and custom skills while advancing a recorded baseline.
- **`/vision`** — explicitly send an image to a configured external vision provider. Use it only when you want an external endpoint instead of the current model's native vision; the agent never invokes it automatically.
- **`/cp`** — commit and push the current changes, then walk through pull, merge, conflict resolution, and another push if the remote is ahead.

## Precondition

**`/setup-matt-pocock-skills`** — run before your first engineering flow to configure the issue tracker, triage labels, doc layout, image-understanding guidance, and shared Claude/Codex instructions the other skills assume. It keeps `AGENTS.md` canonical and `CLAUDE.md` as its one-line import. Custom issue trackers also work.
