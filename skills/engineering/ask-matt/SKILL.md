...
- **`/report-from-commits`** turns git history since a date into a copy-pastable, non-technical report grouped by feature, with the total commit count and each involved author's commit count and percentage in its opening line. Reach for it when you need a client update, stakeholder recap, or progress report from commits. Accepts relative dates ("最近一个礼拜", "last week") and resolves them automatically; only asks for clarification on anchorless phrases like "recently".
...
- **`/new-raw`** captures a requirement exactly as it arrived, without clarification or dialogue, into the configured issue tracker or a local Markdown fallback. Use it when preservation comes before refinement; `/triage` can shape the captured issue later.
