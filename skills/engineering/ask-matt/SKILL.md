...
- **`/report-from-commits`** — turn git history since an exact date into a copy-pastable, non-technical report grouped by feature. Reach for it when you need a client update, stakeholder recap, or progress report from commits; it refuses to guess dates, so have a `YYYY-MM-DD` ready.
