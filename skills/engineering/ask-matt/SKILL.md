...
- **`/report-from-commits`** — turn git history since a date into a copy-pastable, non-technical report grouped by feature, with the commit count and involved authors in its opening line. Reach for it when you need a client update, stakeholder recap, or progress report from commits. Accepts relative dates ("最近一个礼拜", "last week") and resolves them automatically; only asks for clarification on anchorless phrases like "recently".
